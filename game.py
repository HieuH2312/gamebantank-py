...
class Game:
    def __init__(self):
        ...
        self.playing_level = 1  # Nhớ map level đang chơi (1-3)
        self.current_wave = 1  # Sóng chơi thực tế, tăng dần qua các vòng lặp
        ...
        self._load_start_screen_image()

    # ------------------------------------------------------------------
    def start_level(self, level: int, wave: int | None = None):
        ...
        self.current_wave = wave if wave is not None else 1
    ...
    def _load_start_screen_image(self):
        base_dir = os.path.dirname(__file__)
    ...
    def _draw_start_screen(self):
        if self.start_screen_image is not None:
        ...
        if (self.blink_timer // 30) % 2 == 0:
            ...
            self.screen.blit(
                sub, sub.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2 + 330))
            )

    def _draw_playing_screen(self):
        self.screen.fill((0, 0, 0))
        self.map.draw(self.screen)
        self.player.draw(self.screen, self.map)

        for enemy in self.enemies:
            enemy.draw(self.screen, self.map)
    ...
    def _create_enemies_for_level(self, wave: int):
        spawn_positions = [
            (3, 2, "Xe tăng địch 1.png"),
            (6, 2, "Xe tăng địch 2.png"),
            (9, 2, "Xe tăng địch 3.png"),
            (12, 2, "Xe tăng địch 1.png"),
            (15, 2, "Xe tăng địch 2.png"),
            (18, 2, "Xe tăng địch 3.png"),
            (21, 2, "Xe tăng địch 1.png"),
            (4, 3, "Xe tăng địch 2.png"),
            (9, 3, "Xe tăng địch 3.png"),
            (14, 3, "Xe tăng địch 1.png"),
            (19, 3, "Xe tăng địch 2.png"),
        ]

        count = min(3 + max(0, wave - 1), 11)
