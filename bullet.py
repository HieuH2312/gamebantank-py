...
from obstacle import Empty, Forest, Base  # ✅ import class thay vì dict
...
class Bullet(pygame.sprite.Sprite):
    ...
    def update(self, game_map):
        ...
        # ✅ dùng isinstance thay so sánh số nguyên
        if isinstance(tile, (Empty, Forest)):
            return

        if isinstance(tile, Base):
            self.hit_base = True
            game_map.set_tile(col, row, Empty)  # ✅ truyền class, set_tile sẽ gọi ()
            self.alive = False
            return

        # ✅ dùng thuộc tính .destructible thay DESTRUCTIBLE dict
        if tile.destructible:
            game_map.set_tile(col, row, Empty)
